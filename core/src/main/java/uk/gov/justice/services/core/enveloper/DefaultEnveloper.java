...
public class DefaultEnveloper implements Enveloper {
    ...
    private Metadata buildMetaData(final Metadata metadata, final String name, final Clock clock) {
        ...
        metadata.streamId().ifPresent(uuid -> metadataBuilder.add(STREAM, getJsonBuilderFactory().createObjectBuilder().add(STREAM_ID, uuid.toString()).build()));
    ...
    private JsonArray createCausation(final Metadata metadata) {
        final JsonArrayBuilder causation = getJsonBuilderFactory().createArrayBuilder();
